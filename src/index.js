 ...
 */
async function processStream(reader, decoder, writer) {
	try {
		let buffer = '';
...
export default {
	async fetch(request, env, ctx) {
		...
		if (url.pathname === '/deepseek/api') {
			...
			try {
				const { messages, model = 'deepseek-chat', token = '' } = await request.json();
				...
				if (!response.ok) {
					...
					return new Response(`DeepSeek API error: ${response.status}`, { status: response.status });
				}
				// 创建SSE输出数据
				...
				processStream(reader, decoder, writer);
